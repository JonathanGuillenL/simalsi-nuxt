...
const { pending, data } = useFetch<HomeData>('/api/home/data', {
...
watch(pending, () => {
  ...
  totales.totalClientes = data.value?.count.totalClientes || 0;
  totales.totalFacturas = data.value?.count.totalFacturas || 0;
  totales.totalServicios = data.value?.count.totalServicios || 0;
  totales.totalSolicitudes = data.value?.count.totalSolicitudes || 0;
  ...
  optionsBar.value = {
    ...
    xAxis: {
      type: 'category',
      data: data.value?.count.solicitudCount.map(item => `${item.month}/${item.year}`)
    },
    yAxis: {
      type: 'value'
    },
    series: [
      {
        data: data.value?.count.solicitudCount.map(item => item.count),
        type: 'bar'
      }
    ]
  }
    ...
    series: [
      {
        ...
        data: [
          ...
          { name: "Región Autónoma de la Costa Caribe Norte", value: 0 },
            ...
            const de = data.value?.count.departamentoClinica.find(d => d.departamento == item.name)
            console.log(de)
    ...
    series: [
      {
        ...
        data: data.value?.count.servicioRank.map(item => ({
          name: item.servicio,
          value: item.cantidad
        }))
      }
