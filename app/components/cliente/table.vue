...
const { data } = await useLazyFetch<Page<any>>('/api/cliente/page', {
...
function mapTipoCliente(tipo: string) {
    const tipos = [
        { value1: 'CLIENTE_ESPONTANEO', value2: 'Cliente espontáneo' },
        { value1: 'MEDICO_AFILIADO', value2: 'Médico afiliado' },
        { value1: 'CLINICA_AFILIADA', value2: 'Clínica afiliada' }
    ]

    return tipos.find(t => t.value1 == tipo)?.value2
}
</script>
    ...
    <table class="text-left w-full">
      ...
      <tbody class="bg-white">
        <tr v-for="item in data?.data" :key="item.id" class="border-b hover:bg-gray-50">
          ...
          <td class="border px-6 py-3">
            {{ mapTipoCliente(item.tipoCliente) }}
          </td>
          <td class="border px-6 py-3">
