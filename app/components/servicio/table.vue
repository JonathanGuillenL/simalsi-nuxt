...
const criterioItems = [
  ...
  { title: 'Región anatómica', value: 'regionAnatomica' },
  { title: 'Estado', value: 'estado' },
]

const { data } = await useLazyFetch<Page<ServicioPageResponse>>('/api/servicio/page', {
    ...
    <table class="text-left w-full">
      <thead class="text-sm bg-gray-100">
        <tr>
          ...
          <th class="px-6 py-3">Región anatómica</th>
      ...
      <tbody class="bg-white">
        <tr v-for="item in data?.data" :key="item.id" class="border-b hover:bg-gray-50">
          ...
          <td class="border px-6 py-3">
            {{ item.procedimientoQuirurgico?.descripcion }}
          </td>
          <td class="border px-6 py-3">
            {{ item.procedimientoQuirurgico?.regionAnatomica?.descripcion }}
          </td>
          <td class="border px-6 py-3">
